<?php
	...
	if (isset($_GET['action']) && $_GET['action'] == 'view') {
		$sql = "SELECT * FROM tai_khoan ORDER BY ten";
	...
	if (isset($_POST['action']) && $_POST['action'] == 'edit') {
		...
		$rs = mysqli_query($db, "SELECT * FROM tai_khoan WHERE LOWER(ten)=LOWER('$ten_moi') AND id != $ma_kh");
		$trungten = false;
		...
		$rs = mysqli_query($db, "SELECT * FROM tai_khoan WHERE sdt='$sdt_moi' AND id != $ma_kh");
		$trungsdt = false;
		if (mysqli_num_rows($rs) == 1) {
			$trungsdt = true;
		...
		if (!$trungten && !$trungsdt) {
			$rs = mysqli_query($db, "UPDATE tai_khoan SET ten='$ten_moi', sdt='$sdt_moi' WHERE id='$ma_kh'");
	...
	if (isset($_POST['action']) && $_POST['action'] == 'add') {
		...
		$rs = mysqli_query($db, "SELECT * FROM tai_khoan WHERE LOWER(ten)=LOWER('$ten')");
		$trungten = false;
		...
		if (!$trungten && !$trungsdt) {
			$rs = mysqli_query($db, "INSERT INTO tai_khoan(ten, sdt) VALUES('$ten','$sdt')");
