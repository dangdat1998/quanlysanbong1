...
function resetTables(){
	...
	$("#ds_datsan_khach").html(LOADING_ORDERS);
	$("#ds_datsan_lichsu").html(LOADING_ORDERS);
...
function xemDsDatSanKhach(day) {
	resetTables();
	$.ajax({
		url: "/quanlysanbong/api/xemdatsankhach.php",
		type: "GET",
		cache: false,
		data: {
			action: "xemdatsankhach",
			day: day
		},
		success: function(json) {
			var data = $.parseJSON(json);
			$("#tieudeds_khach").html(getCurrentFormattedDate());
			// $("#tieudetime").html(getCurrentFormattedDate());
			veTableDatSanKhach(data);
			checkInputs();
			// console.log('aa');
		},
		error: function() {
...
function veTableDatSan(data) {
	...
	html += "<thead class='thead-listdatsan'><tr><th>#</th><th>Tên KH</th><th>SĐT</th><th>Sân</th><th>Bắt đầu</th><th>Kết thúc</th><th>Phút</th><th>Đơn giá (/phút)</th><th>Tiền</th><th>Đã thanh toán</th><th>Thanh toán</th><th>Xóa</th></tr></thead>";
	html += "<tbody class='tbody-listdatsan'>";
	var tong_tien = 0;
	var da_thanh_toan = 0;
	...
	for (var i = 0; i < data.length; i++) {
		...
		if (thanh_toan == "0") {
			html += "<td><button class='btnThanhToan btn btn-thanhtoan' datsan_id='" + data[i].datsan_id + "'>Thanh toán</button></td>";
			html += "<td><button class='btnXoaDatSan btn btn-xoa' datsan_id='" + data[i].datsan_id + "'>Xóa</button></td>";
		} else {
			html += "<td><button class='btnThanhToan btn btn-dathanhtoan' datsan_id='" + data[i].datsan_id + "'>Hủy thanh toán</button></td>";
			html += "<td><button disabled class='btnXoaDatSan btn btn-xoa' datsan_id='" + data[i].datsan_id + "'>Xóa</button></td>";
		}
	...
	html += "</tbody>";
	html += "<tfoot class='tfood-listdatsan'>";
	html += "<tr><td></td><td></td><td></td><td></td><td></td><td></td><td colspan=2 class='ttoan-list'><b>Đã thanh toán</b></td><td style='font-weight:bold;color:green;'>" + formatMoney(da_thanh_toan) + "đ</td><td></td><td></td><td></td></tr>";
	html += "<tr><td></td><td></td><td></td><td></td><td></td><td></td><td colspan=2 class='ttoan-list'><b>Chưa thanh toán</b></td><td style='font-weight:bold;color:red;'>" + formatMoney(chua_thanh_toan) + "đ</td><td></td><td></td><td></td></tr>";
	html += "<tr><td></td><td></td><td></td><td></td><td></td><td></td><td colspan=2 class='ttoan-list'><b>Tổng tiền</b></td><td style='font-weight:bold;color:blue;'>" + formatMoney(tong_tien) + "đ</td><td></td><td></td><td></td></tr>";
	html += "</tfoot>";
	html += "</table>";
	$("#ds_datsan").html(html);
...
function veTableDatSanKhach(data) {
	var html = "";
	html +="<link href='https://cdn.jsdelivr.net/npm/bootstrap@5.2.0-beta1/dist/css/bootstrap.min.css' rel='stylesheet' integrity='sha384-0evHe/X+R7YkIZDRvuzKMRqM+OrBnVFBL6DOitfPri4tjfHxaWutUpFmBp4vmVor' crossorigin='anonymous'>";
	html +="<link rel='stylesheet' type='text/css' href='css/common.css' />";
	html += "<table class='table'>";
	...
	for (var i = 0; i < data.length; i++) {
		...
		html += "<td><center>" + status + "</center></td>";
		if (thanh_toan == "0") {
			html += "<td><button disabled class='btnThanhToan btn btn-thanhtoan' datsan_id='" + data[i].datsan_id + "'>Chưa thanh toán</button></td>";
			html += "<td><button class='btnXoaDatSan btn btn-xoa' datsan_id='" + data[i].datsan_id + "'>Xóa</button></td>";

		} else {
			html += "<td><button disabled class='btnThanhToan btn' datsan_id='" + data[i].datsan_id + "'>Đã thanh toán</button></td>";
			html += "<td><button disabled class='btnXoaDatSan btn btn-xoa' datsan_id='" + data[i].datsan_id + "'>Xóa</button></td>";
	...
	html += "</tbody>";
	html += "<tfoot class='tfood-listdatsan'>";
	html += "<tr><td></td><td colspan=2 style='text-align:right; font-style:italic; color: red'><b>Thanh toán liên hệ</b></td><td style='color: red; font-style: italic'>0984380888</td><td></td><td></td><td colspan=2 class='ttoan-list'><b>Đã thanh toán</b></td><td style='font-weight:bold;color:green;'>" + formatMoney(da_thanh_toan) + "đ</td><td></td><td></td><td></td></tr>";
	html += "<tr><td></td><td></td><td></td><td></td><td></td><td></td><td colspan=2 class='ttoan-list'><b>Chưa thanh toán</b></td><td style='font-weight:bold;color:red;'>" + formatMoney(chua_thanh_toan) + "đ</td><td></td><td></td><td></td></tr>";
	html += "<tr><td></td><td></td><td></td><td></td><td></td><td></td><td colspan=2 class='ttoan-list'><b>Tổng tiền</b></td><td style='font-weight:bold;color:blue;'>" + formatMoney(tong_tien) + "đ</td><td></td><td></td><td></td></tr>";
	html += "</tfoot>";
	html += "</table>";
	$("#ds_datsan_khach").html(html);
...
function xemDsLichsu(day) {
	resetTables();
	$.ajax({
		url: "/quanlysanbong/api/xemlichsu.php",
		type: "GET",
		cache: false,
		data: {
			action: "xemlichsu",
		...
		success: function(json) {
			var data = $.parseJSON(json);
			// $("#tieudeds-lichsu").html(getCurrentFormattedDate());
			// $("#tieudetime").html(getCurrentFormattedDate());
			veTableLichsu(data);
			checkInputs();
		},
